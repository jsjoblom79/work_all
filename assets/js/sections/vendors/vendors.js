...
export default async function displayVendor(vendorId){
    ...
    if(vendor !== null){
        ...
        const cdField = await displayField('Created','vendor-create_date','date','gs-input',returnISODate(vendor.create_date));
        const udField = await displayField('Last Updated', 'vendor-modify_date', 'date', 'gs-input', returnISODate(vendor.modify_date));
